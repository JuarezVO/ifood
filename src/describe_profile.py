...
def describe_profile(df: pd.DataFrame) -> None:
    ...
    offer_type = df.groupby('offer_type')['offer_success'].mean() * 100
    offer_type = pd.DataFrame({'Success Rate': offer_type, 'Fail Rate': 100 - offer_type})
    offer_type.plot(kind='bar', ax=axs[1], color=['forestgreen', 'firebrick'], rot=0)
